<?php
	...
	while($row = $query->fetch_assoc()){
		...
		while($crow = $cquery->fetch_assoc()){
			...
			$option .= '
				<li>
					'.$input.'<button class="btn btn-primary btn-sm btn-flat clist"><i class="fa fa-search"></i> Description</button><img src="'.$image.'" height="100px" width="100px" class="clist"><span class="cname clist">'.$crow['fname'].'</span>
				</li>
			';
		}
		...
		$sql = "UPDATE polls SET priority = '$num' WHERE id = '".$row['id']."'";
		$conn->query($sql);
